...
const TableV8: React.FC<ITableV8> = React.forwardRef(
  (
    ...
    const [tableColumns, setTableColumns] = useState<typeof columns>(() => [
      ...columns,
    ]);
    ...
    useEffect(() => {
      if (editMode === false && prevMode && prevMode.current === true) {
        ...
        typeof onHideColumnCheck === "function" &&
          onHideColumnCheck(updatedHiddenColumns);
        ...
        typeof onColumnResize === "function" &&
          onColumnResize(updatedColumnSize);
    ...
    const reorder = useCallback(
      (item: any, newIndex: any) => {
        const { index: currentIndex } = item;
        let currentColumnOrder: any = columnOrder;
        if (columnOrder.length === 0) {
          currentColumnOrder = instance
            .getAllColumns()
            .map((col: any) => col.id)
            .reduce(getColumnsId, []);
        }
    ...
    useEffect(() => {
      if (rowSelection && onCheckRow && typeof onCheckRow === "function") {
        const selectedRowData = instance
          .getSelectedRowModel()
    ...
    const TableHeader = useCallback(() => {
      ...
      return editMode ? (
        ...
        >
          {instance.getAllLeafColumns().map((column: any, i: any) => {
            ...
            return (
              <div
                ...
                className={
                  columnAlign
                    ? `th edit-th ${canEdit && "editable"} ${
                        columnAlign[column.id] || "right"
                      }`
                    : `th edit-th ${canEdit && "editable"}`
                }
                style={{
                  ...
                  flex: `${
                    header ? header.getSize() : column.getSize()
                  } 0 auto`,
                }}
              >
                {canEdit && (
          ...
          {deleteMode && <div className="th delete-column">&nbsp;</div>}
        </StyledTableHeader>
      ) : (
        <>
              ...
              {headerGroup.headers.map((header: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    {header.column.getCanSort() && (
                      <i className={sortIcon(header.column.getIsSorted())}></i>
                    )}
                  </div>
                );
              })}
    ...
    const pageStartEnd = useMemo(() => {
      ...
      let paginationEnd: number =
        instance.getPageCount() > PC_LIMIT ? PC_LIMIT : instance.getPageCount();
      if (paginationStyle === "number") {
        if (pagination && instance.getState().pagination.pageSize) {
          if (instance.getState().pagination.pageIndex > 5) {
            // TODO: this is because when more than 10 pages, only the 6th items get center
            if (
              ...
              paginationStart =
                instance.getState().pagination.pageIndex - 4 < 0
                  ? 1
                  : instance.getState().pagination.pageIndex - 4;
              paginationEnd = instance.getState().pagination.pageIndex + 5;
            } else {
              paginationStart =
                instance.getPageCount() - 9 < 0
                  ? 1
                  : instance.getPageCount() - 9;
    ...
    return (
      <div>
        ...
        >
          <TableWrapper className={`table ${editMode ? "edit-table" : ""}`}>
            {hasHeader && <TableHeader />}
            {instance.getRowModel().rows.map((row: any, index: number) => {
              const rowData = editMode
                ? row.getAllCells()
                : row.getVisibleCells();
              const rowId = row.original.id;
              const rowClass = expanded === rowId ? "tr active-row" : "tr";
              return dndMode ? (
                <DnDRow key={rowId} index={index} row={row} moveRow={moveRow} />
              ) : (
                <React.Fragment key={rowId}>
                  <div
                    ...
                    onContextMenu={() => {
                      if (
                        onRowContextClick &&
                        typeof onRowContextClick === "function"
                      ) {
          ...
          {pagination &&
            ...
            paginationStyle === "number" && (
              <StyledPaginationControl className="PaginationControl">
                ...
                {range(
                  pageStartEnd.paginationStart,
                  pageStartEnd.paginationEnd
                ).map((i: any) => (
                  <span
                    key={i}
                    className={
                      i === instance.getState().pagination.pageIndex + 1
                        ? `active`
                        : ""
                    }
                    onClick={() => {
                  ...
                  <ActionText
                    ...
                    onClick={() => {
                      if (
                        ...
                        instance.getState().pagination.pageIndex > 0
                      ) {
                        instance.previousPage();
                      }
                    }}
