...
export const StyledBaseButton = styled(({
  ...
  border: ${({ borderColor, disabled }) => `1px solid ${disabled ? '#E6E6E6' : borderColor}`};
  ...
  outline: none;
  white-space: nowrap;
  ${({ colorGradient, disabled }) => (colorGradient && !disabled)
    ? css`
      background-color: transparent;
      background-image: linear-gradient(to bottom, ${colorGradient[0]}, ${colorGradient[1]});`
    : null
  };
`;
...
const BaseButton: React.FC<BaseButtonProps> = ({
  ...
  return (
    <StyledBaseButton {...rest} {...baseButtonProps} onClick={onClick} onMouseOver={onMouseOver} onMouseLeave={onMouseLeave}>
      {text}
    </StyledBaseButton>
  );
};
