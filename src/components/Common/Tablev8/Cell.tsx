...
const Cell: React.FC<CellProps> = React.memo(
  ({ type, value, align = "right", bold = false, color, style }) => {
    const { row1 = EMPTY_VALUE, row2 = EMPTY_VALUE } = value;
    ...
    switch (type) {
      case "2row":
        render = (
          <>
            <StyledCell align={align} bold={bold} color={color} style={style}>
              {row1 || EMPTY_VALUE}
            </StyledCell>
            <StyledCell align={align} bold={bold} color={color} style={style}>
              {row2 || EMPTY_VALUE}
    ...
    return <Styled2Row align={align}>{render}</Styled2Row>;
  }
);
